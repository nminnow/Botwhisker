...
def get_next_field(field, text):
    fields = ['kittypet_name', 'kit_name', 'apprentice_name', 'warrior_name', 'medicine_cat_name',
        'queen_name', 'deputy_name', 'leader_name', 'elder_name', 'father']
...
for title in zh2cp.characters:
    if title == '闪电': # last before interruption
        start = True
    if start:
        print(title)
        ...
        for clm in item.claims['P83']:
            ...
            pos_end = page.text.index('|{0}'.format(get_next_field(
                field, page.text)))
